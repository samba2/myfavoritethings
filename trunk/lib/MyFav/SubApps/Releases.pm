...
sub runModeProcessCodeStatus {
	...
	if ($inputError) {
		...
		return $self->runModeCheckCodeStatus("The code '$downloadCode' is not existing.");
	}
	...
	if (! $releaseDb->codeIsUsed($downloadCode)) {
		...
		$template->param( "CODESTATUS" => "The code was used and is still inside the download frame." );
	}
	else {
